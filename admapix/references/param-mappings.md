...
## Industry / 行业 (trade_level1)

Level-1 industry codes (source: `GET /api/data/filter-options` → `tradeLevelTree`). Pass as `trade_level1` (string array). Unrecognized codes are silently ignored, so use these exact values:

| User says (EN) | User says (CN) | trade_level1 |
|---|---|---|
| Tools | 工具 | "601" |
| Game | 游戏 | "602" |
| E-commerce | 电商 | "603" |
| Entertainment | 泛娱乐 | "604" |
| Life Services | 生活服务 | "605" |
| Education | 教育 | "606" |
| Finance | 金融 | "607" |
| Business | 商务 | "608" |
| Health & Fitness | 健康 | "609" |
| Popular | 热门行业 | "700" |

For level-2/3 sub-industries, fetch `GET /api/data/filter-options` (`tradeLevel`, `tradeLevel2`, `tradeLevelTree`).

## Page Size / 每页数量

`page_size` is capped at **10** — never exceed 10; clamp any larger request down to 10. Use `page` to get more results.

| User says (EN) | User says (CN) | page_size |
|---|---|---|
| default | 默认 | 10 |
| show more / lots / maximum | 多看一些 / 多看 / 最多 | 10 (hard cap) |
| show fewer / brief | 少看几条 / 简要 | 1–9 (as asked, ≤10) |
